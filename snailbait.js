...
this.SNAIL_PACE_VELOCITY = 50,
 ...
 // Platform scrolling offset (and therefore perceived
 // game speed) is PLATFORM_VELOCITY_MULTIPLIER *
 // backgroundOffset. The platforms move
 // PLATFORM_VELOCITY_MULTIPLIER times as fast as the
 // background
 ...
 this.toastElement = document.getElementById('snailbait-toast'),
  ...
  this.BAT_CELLS_HEIGHT = 34, // Bat cell width varies
  ...
  this.beeData = [
    {left: 200,  top: this.TRACK_1_BASELINE - 1.5*this.BEE_CELLS_HEIGHT},
    {left: 350,  top: this.TRACK_2_BASELINE - 1.5*this.BEE_CELLS_HEIGHT},
    {left: 550,  top: this.TRACK_1_BASELINE - this.BEE_CELLS_HEIGHT},
    {left: 750,  top: this.TRACK_1_BASELINE - 1.5*this.BEE_CELLS_HEIGHT},
    {left: 924,  top: this.TRACK_2_BASELINE - 1.75*this.BEE_CELLS_HEIGHT},
    {left: 1500, top: 225},
    {left: 1600, top: 115},
    {left: 2225, top: 125},
    {left: 2295, top: 275},
  ...
  this.sprites      = [],   // For convenience, this contains
                            // all sprites from preceeding arrays
...
SnailBait.prototype = {
  ...
  createSnailSprites: function(){
    ...
    for(var i=0; i < this.snailData.length; i++){
      snail = new Sprite('snail',
                          snailArtist,
                          [this.paceBehavior, this.snailShootBehavior, new CycleBehavior(300, 1500)]);
  ...
  draw: function(now){
    ...
    this.drawSprites();
    // No longer need drawRunner() or drawPlatforms() since they are sprites
  },
  ...
  drawBackground: function(){
    ...
    // Translate everything by the background offset
  	...
  	// Initially onscreen - 1st Background:
  	...
  	// Initially offscreen - 2nd Background:
  	this.context.drawImage(
      this.spritesheet, 0, BACKGROUND_TOP_IN_SPRITESHEET,
      this.BACKGROUND_WIDTH, this.BACKGROUND_HEIGHT,
  ...
  fadeOutElements: function(){
    ...
    for(var i=0; i < args.length-1; i++){
      args[i].style.opacity = snailBait.TRANSPARENT;   // opacity: 0;
    }
  ...
  animate: function(now){
    if(snailBait.paused){
      ...
      snailBait.draw(now);
      snailBait.lastAnimationFrameTime = now;			// Draw an animation frame
      requestNextAnimationFrame(snailBait.animate);		// Call animate() again
    }
...
// Turn left and right from 'd/k' or 'left/right'
...
window.addEventListener(
    'focus', function(e){
      var originalFont = snailBait.toastElement.style.fontSize,  // Save to restore later
      ...
      else if(snailBait.paused){       // If game is paused
        ...
        if(takeAction()){
          ...
          setTimeout(function(e){
            ...
            setTimeout(function(e){
              ...
              setTimeout(function(e){
                if(takeAction()){
                  snailBait.togglePaused(); // Resume gameplay
